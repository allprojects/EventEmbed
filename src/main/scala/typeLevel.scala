...
object HListOps {
  ...
  object Join {
    ...
    implicit def baseCase[L1 <: HList, L2 <: HList, Out0 <: HList](
      implicit prepend: Prepend[L1, L2]) =
      new Join[L1, L2, HNil] {
  ...
  object LTEqCom {
    ...
    implicit def ltEqCom2[NA1 <: Nat, NA2 <: Nat, NB1 <: Nat, NB2 <: Nat](
      implicit a : NA1 <= NB1,
               b : NA2 <= NB2) =
      new LTEqCom[Compare[NA1,NA2], Compare[NB1,NB2]]{};
  ...
  object ComIsSorted {
    ...
    implicit def hlistNonDecreasing2[Com1 <: Compare[_,_], Com2 <: Compare[_,_], T <: HList] (
      implicit ltEq : LTEqCom[Com1,Com2],
               ndt : ComIsSorted[Com2 :: T]) =
