 ...
 * 
 */
class EventNodeJoin[T <: Product, U <: Product, Z, N1 <: Nat, N2 <: Nat](ev1: Event[T],
                                                   ev2: Event[U],
                                                   window1: String,
                                                   window2: String,
                                                   condition: Compare[N1,N2])
  extends EventNode[Z] {
...
case class DSLWindowJoin[T <: Product](event: Event[T], window: DSLWindow) {
  ...
  def join[U <: Product, RES <: Product](other : DSLWindowJoin[U])(where : (T,U) => Boolean)(pi : (T,U) => RES) = 
